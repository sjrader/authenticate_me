    ...
    return (
       <div>
        <div>
         ...
         <div>
            <h1>My next ten upcoming events:</h1>
             {myEvents.map(myEvent => (
                 <div key={myEvent.id}>
                 <h2><a href={`/events/${myEvent.Event.id}`}>{myEvent.Event.title}</a></h2>
                 <h4>On {myEvent.Event.date} from {myEvent.Event.startTime} until {myEvent.Event.endTime}</h4>
                 {/* <a href={`/venues/${event.venueId}`}>Located at {event.Venue.name}</a> */}
                 {/* Cannot bring in the information about the venue with the current search */}
                 </div>
             ))}
         </div>
       </div>
